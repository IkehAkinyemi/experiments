...
func main() {
	tp, tpErr := tracing.TraceProvider()
	if tpErr != nil {
		log.Fatal(tpErr)
	}
...
func connectMongo() {
	...
	docs := []interface{}{
		bson.D{{Key: "id", Value: "1"}, {Key: "title", Value: "Buy groceries"}},
		bson.D{{Key: "id", Value: "2"}, {Key: "title", Value: "install Aspecto.io"}},
		bson.D{{Key: "id", Value: "3"}, {Key: "title", Value: "Buy dogz.io domain"}},
	}
